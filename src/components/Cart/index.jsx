...
const Cart = () => {
  ...
  const { cartItems, generateOrder } = useContext(CartContext);

  /* Cada vez que se modifica el carrito, actualizamos la cantidad de productos */
  useEffect(() => {
    setProductsLength(
      cartItems?.reduce(() => cartItems.length, 0)
    );
  }, [cartItems]);
    ...
    <div className={styles.cartContainer}>
      ...
      {cartItems && cartOpen && (
        <div className={styles.cart}>
          ...
          {cartItems.length === 0 ? (
            <p className={styles.cartVacio}>empty shopping cart</p>
          ) : (
          ...
          {cartItems.length > 0  ? (
          <button onClick={() => generateOrderLocal()} className={styles.buttons}>Generate order</button>
          ) : ""} 
